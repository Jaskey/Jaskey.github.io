...
原因是开发者**担心定时任务在同一时刻被触发多次**，导致业务有问题。实际上这是对于框架最基本的原理不了解。在官方文档的功能列表里http://elasticjob.io/docs/elastic-job-lite/00-overview/，一个最基本的功能就是：

> 作业分片一致性，保证同一分片在分布式环境中仅一个执行实例



Elastic Job会依赖zookeeper选举出对应的实例做sharding，从而保证只有一个实例在执行同一个分片（如果任务没有采取分片，及分片数是0，就意味着这个任务只有一个实例在执行）
